...
class GraphicDisplay(tk.Tk):
    ...
    def move_by_policy(self):
        movement_count = 0 #code injection
        ...
        if self.improvement_count != 0 and self.is_moving != 1:
            ...
            while (x,y) != (2,2): #code injection box is not in 2,2
            #while len(self.agent.get_action([x, y])) != 0:
                if movement_count ==3: ##code injection
                    action = 1 #DOWN
                else:
                    action = random.sample(self.agent.get_action([x, y]), 1)[0] ##code injection
                self.after(100, self.rectangle_move(action)) #100 -> 200
                x, y = self.find_rectangle()
                self.is_moving = 0
                movement_count += 1
